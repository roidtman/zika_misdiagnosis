...
load('../output/3b_revised_p_temporal_aggregated_confirmed-igm.RData')
...
f = '../output/4b_p_distribution_overall_confirmed-igm.RData'
if(!file.exists(f)){
  for(cc in 1:length(countries)){
    if(cc != 38){
...
# pdf('~/Dropbox/misdiagnosis/output/results_final/supp_4b_revised_p_distributions_cumulative_cases_confirmed-igm.pdf',
#     height = 6, width = 6)
# countries_keep = 1:length(countries)
# countries_keep = countries_keep[-c(20, 35, 38)]
...
# pdf('../output/supp/supp_violin_temporally_aggregated_confirmed-igm.pdf', width = 8, height = 6)
layout(matrix(1:3, nrow = 3))
...
# ordered by end_prop
# pdf('../supp/supp_b_revised_prop_z_cumulative_cases_confirmed-igm.pdf', height = 5, width = 8)
...
vioplot(
  ...
  end_prop_samples[,24], 
  end_prop_samples[1:100,35], end_prop_samples[1:100,39],
  ...
  end_prop_samples[,28], end_prop_samples[1:100,41],
  end_prop_samples[1:100,42], end_prop_samples[,15], 
  end_prop_samples[,19], end_prop_samples[1:100,40], 
  end_prop_samples[,21], end_prop_samples[1:100,38],
  end_prop_samples[,26], end_prop_samples[,25], 
  end_prop_samples[,4], end_prop_samples[,18],
  end_prop_samples[,33], end_prop_samples[,29], 
  end_prop_samples[,36], end_prop_samples[1:100,43], 
  at = x,
