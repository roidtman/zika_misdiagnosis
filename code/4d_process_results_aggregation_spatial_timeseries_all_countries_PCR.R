...
#=============================================================================#
# monte carlo samples for the number of missed infections from 2016-2017
#=============================================================================#

# for each sample, calculate number of missed infections
...
num_samples = 1000
mc_samples_p_Zc = rep(NA, num_samples)
mc_samples_p_Zs = rep(NA, num_samples)


samps = sample(p_Zc, num_samples, replace = T)
mc_samples_p_Zc = calc_missed_zika(c_Z_in = sum(zik_c[,-(1:2)]),
                              c_O_in = sum(other_c[,-c(1:2)]),
                              p_z_in = samps, browse = F) 

samps = sample(p_Zs, num_samples, replace = T)
mc_samples_p_Zs = calc_missed_zika(c_Z_in = sum(zik_s[,-c(1:2)]),
                              c_O_in = sum(other_s[,-c(1:2)]),
                              p_z_in = samps, browse = F) 


summary(mc_samples_p_Zc)
summary(mc_samples_p_Zs)

mc_samples_sum_total = mc_samples_p_Zc + mc_samples_p_Zs
