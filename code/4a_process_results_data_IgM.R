...
load('../output/3e_revised_case_estimates_monthly_agg_confirmed-igm.RData')
...
f = '../output/4e_p_distribution_overall_confirmed-igm.RData'
if(!file.exists(f)){
...
# pdf('../output/supp/supp_4e_revised_p_distributions_monthly_agg_confirmed-igm.pdf', height = 6, width = 6)
...
#=============================================================================#
# looking at venezuela, jamaica, brazil, and bolivia
#=============================================================================#

countries_subset = c('VEN', 'JAM', 'BRA', 'BOL')

mc_samples_sum_countries_subset = sapply(1:length(countries_subset), 
                                         function(ff) sum(mc_samples[ff,,which(countries == countries_subset[ff])], na.rm = T))

reported_cases_countries_subset = sapply(1:length(countries_subset),
                                         function(ff) sum(zik_c_month[,which(colnames(zik_c_month) == countries_subset[ff])],
                                                          zik_s_month[,which(colnames(zik_c_month) == countries_subset[ff])]))
...
# pdf('..output/supp/supp_4e_revised_prop_with_barplot_monthly_agg-confirmed_igm.pdf', height = 5, width = 8)
...
end_prop_samples_IgM = end_prop_samples
end_prop_sample_cum_IgM = end_prop_sample_cum
save(end_prop_samples_IgM, 
     end_prop_sample_cum_IgM, 
     file = '../output/figures_out/fig5_IGM.RData')
...
# pdf('../output/supp/supp_4e_revised_estimates_timeseries_monthly_agg_confirmed-igm.pdf', 
#     width = 8.5, height = 6)
layout(matrix(c(1,1,2,2,2), nrow = 5))
...
# SAVE OUTPUT FOR FIGURE
revised_zika_CI_IgM = revised_zika_CI
time_missed_zika_region_IgM = time_missed_zika_region
save(revised_zika_CI_IgM, time_missed_zika_region_IgM, 
     file = '../output/figures_out/fig4_IGM.RData')
