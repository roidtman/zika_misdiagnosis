...
observed_p_prime_Zc = sum(zik_c[,-c(1:2)]) / sum(zik_c[,-c(1:2)], other_c[,-c(1:2)])
observed_p_prime_Zs = sum(zik_s[,-c(1:2)]) / sum(zik_s[,-c(1:2)], other_s[,-c(1:2)])
...
p_prime_Zc = bb_conj(alpha_in = alpha_prior, beta_in = beta_prior,
                     num_suc = sum(zik_c[,-c(1:2)]),
                     num_tri = sum(zik_c[,-c(1:2)], other_c[,-c(1:2)]))

p_prime_Zs = bb_conj(alpha_in = alpha_prior, beta_in = beta_prior,
                     num_suc = sum(zik_s[,-c(1:2)]),
                     num_tri = sum(zik_s[,-c(1:2)], other_s[,-c(1:2)]))
...
p_prime_Zc_samples = rep(NA, num_samples)
p_prime_Zs_samples = rep(NA, num_samples)

p_prime_Zc_samples = rbeta(num_samples, shape1 = p_prime_Zc[1], shape2 = p_prime_Zc[2])
p_prime_Zs_samples = rbeta(num_samples, shape1 = p_prime_Zs[1], shape2 = p_prime_Zs[2])
...
diag_c_time_country_1_2 = lapply(1:num_samples, function(ff) 
  diag_dist_igg[lapply(p_prime_Zc_samples, constrain_diag_1_2, diag_dist_igg)[[ff]],])

diag_s_time_country_1_2 = lapply(1:num_samples, function(ff) 
  diag_dist_s[lapply(p_prime_Zs_samples, constrain_diag_1_2, diag_dist_s)[[ff]],])
...
diag_c_time_country_3 = 
  lapply(1:num_samples, function(kk) 
    matrix(diag_c_time_country_1_2[[kk]],ncol=2)[lapply(1:num_samples, 
                                                        function(ff) constrain_diag_3(p_prime_Zc_samples[ff], diag_c_time_country_1_2[[ff]]))[[kk]],])

diag_s_time_country_3 = 
  lapply(1:num_samples, function(kk) 
    matrix(diag_s_time_country_1_2[[kk]],ncol=2)[lapply(1:num_samples, 
                                                        function(ff) constrain_diag_3(p_prime_Zs_samples[ff], diag_s_time_country_1_2[[ff]]))[[kk]],])
...
for(ii in 1:num_samples){
  out_tmp = calc_pZ(se_in = matrix(diag_c_time_country_3[[ii]], ncol = 2)[,1], 
                    ...
                    p_prime_in = p_prime_Zc_samples[ii])
...
for(ii in 1:num_samples){
  out_tmp = calc_pZ(se_in = matrix(diag_s_time_country_3[[ii]], ncol = 2)[,1], 
                    ...
                    p_prime_in = p_prime_Zs_samples[ii], browse = F)
  vec_tmp = c(vec_tmp, out_tmp)
}
p_Zs = unlist(vec_tmp)
...
save(p_Zs, p_Zc, file = '../output/3d_revised_p_temporal_spatial_aggregated_confirmed-igm.RData')
